...
export function normalizeSellerProduct(row = {}) {
  ...
  return {
    ...
    thumbnail: row.thumbnail || images.find((image) => image.isPrimary)?.url || images[0]?.url || "",
    images,
    variants,
...
export function serializeSellerProduct(values = {}) {
...
function invalidateProducts(queryClient) {
  return invalidateCatalogResources(
    queryClient,
    [sellerProductKeys.all, ["admin", "products"], ["catalog", "products"], ["storefront", "stores"]],
    ["/products"],
  );
}
...
export function useCreateSellerProduct() {
  ...
  return useMutation({ mutationFn: createSellerProduct, onSuccess: () => invalidateProducts(queryClient) });
}

export function useUpdateSellerProduct() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: ({ id, values }) => updateSellerProduct(id, values), onSuccess: () => invalidateProducts(queryClient) });
}

export function useDeleteSellerProduct() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: deleteSellerProduct, onSuccess: () => invalidateProducts(queryClient) });
}
