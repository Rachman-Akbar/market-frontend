...
import { validateFields, minimumNumber, required, validUrl } from "@/core/utils/formValidation";
...
function createInitialValues(product) {
  if (!product) {
    return {
      ...
      stock: 0,
      thumbnail: "",
      images: [{ clientId: createClientId("image"), url: "", altText: "" }],
      variants: [{ clientId: createClientId("variant"), id: null, name: "", sku: "", price: 0, stock: 0, values: [] }],
    };
  }
  ...
  return {
    ...
    images: product.images.length
      ...
      : [{ clientId: createClientId("image"), url: product.thumbnail || "", altText: "" }],
...
export function SellerProductEditor({
  ...
  const rules = useMemo(() => ({
    ...
    thumbnail: validUrl("URL thumbnail"),
    price: values.mode === "simple" ? minimumNumber("Harga", 0) : () => "",
    stock: values.mode === "simple" ? minimumNumber("Stok", 0) : () => "",
  }), [isAdmin, values.mode]);
  ...
  const submit = async (event) => {
    ...
    const invalidImage = values.images.find((image) => validUrl("URL gambar")(image.url));
    if (invalidImage) nextErrors.images = "Seluruh URL gambar harus valid.";
    ...
    if (values.mode === "variant") {
      ...
      if (values.variants.some((variant) => Number(variant.price) < 0 || Number(variant.stock) < 0)) {
        nextErrors.variantStock = "Harga dan stok setiap variant tidak boleh kurang dari 0.";
      }
    }
  ...
  return (
    ...
    >
      <form onSubmit={submit}>
        ...
        <div className="min-h-[420px] space-y-6 p-5 sm:p-6">
              ...
              <section className="rounded-xl border border-slate-200 bg-white">
                ...
                <div className="grid gap-4 p-4 md:grid-cols-2">
                  {isAdmin ? (
                    <FormField label="Toko" error={errors.storeId} required>
                      <select value={values.storeId} onChange={(event) => setField("storeId", event.target.value)} className={inputClassName}>
                        ...
                        {stores.map((store) => <option key={store.id} value={store.id}>{toTitleCase(store.name)}</option>)}
                      </select>
                    </FormField>
                  ) : null}
                  ...
                  <FormField label="Kategori" error={errors.categoryId} required>
                    <select value={values.categoryId} onChange={(event) => setField("categoryId", event.target.value)} className={inputClassName}>
                      <option value="">Pilih kategori</option>
                      {sortedCategoryOptions.map((category) => {
                        const depth = getCategoryDepth(category);
                        return (
                          <option key={category.id} value={category.id}>
                            {`L${depth + 1} · ${"— ".repeat(depth)}${toTitleCase(getCategoryName(category))}`}
                          </option>
                        );
                      })}
                    </select>
                  ...
                  <FormField label="Status publikasi">
                    <select value={values.status} onChange={(event) => setField("status", event.target.value)} className={inputClassName}>
          ...
          {activeSection === "images" ? (
            <div className="space-y-5">
              ...
              <FormField label="URL thumbnail" error={errors.thumbnail} hint="Boleh dikosongkan agar memakai gambar pertama.">
                <input value={values.thumbnail} onChange={(event) => setField("thumbnail", event.target.value)} className={inputClassName} placeholder="https://..." />
              </FormField>
            </div>
          ) : null}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50/50 px-5 py-4 sm:px-6">
          <div>
            {product && onDelete ? (
              <button type="button" onClick={() => onDelete(product)} className="h-10 rounded-lg px-4 text-sm font-extrabold text-red-600 hover:bg-red-50">Hapus Produk</button>
            ) : null}
          </div>
          <div className="flex gap-3">
            <button type="button" onClick={onClose} className="h-10 rounded-lg border border-slate-200 bg-white px-5 text-sm font-bold text-slate-600 hover:bg-slate-50">Batal</button>
            <button type="submit" disabled={mutation.isPending} className={`h-10 rounded-lg px-5 text-sm font-extrabold text-white disabled:opacity-60 ${isAdmin ? "bg-teal-600 hover:bg-teal-700" : "bg-emerald-600 hover:bg-emerald-700"}`}>
