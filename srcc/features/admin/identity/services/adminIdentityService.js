...
async function invalidate(queryClient, ...keys) {
...
export function useCreateAdminUser() {
  ...
  return useMutation({ mutationFn: createAdminUser, onSuccess: () => invalidate(queryClient, adminIdentityKeys.users) });
}

export function useUpdateAdminUser() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: ({ id, values }) => updateAdminUser(id, values), onSuccess: () => invalidate(queryClient, adminIdentityKeys.users) });
}

export function useDeleteAdminUser() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: deleteAdminUser, onSuccess: () => invalidate(queryClient, adminIdentityKeys.users) });
}

export function useCreateAdminRole() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: createAdminRole, onSuccess: () => invalidate(queryClient, adminIdentityKeys.roles, adminIdentityKeys.users) });
}

export function useUpdateAdminRole() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: ({ id, values }) => updateAdminRole(id, values), onSuccess: () => invalidate(queryClient, adminIdentityKeys.roles, adminIdentityKeys.users) });
}

export function useDeleteAdminRole() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: deleteAdminRole, onSuccess: () => invalidate(queryClient, adminIdentityKeys.roles, adminIdentityKeys.users) });
}
