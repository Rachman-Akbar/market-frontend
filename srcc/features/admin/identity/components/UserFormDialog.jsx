    ...
    <CrudDialog open={open} onClose={onClose} title={user ? "Edit User" : "Tambah User"} subtitle="Kelola akun, role, verifikasi, active/non-active, dan banned admin." size="max-w-3xl">
      <form onSubmit={submit}>
        <div className="grid gap-4 p-6 md:grid-cols-2">
          ...
          {canBeBanned ? (
            <label className="flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50/40 p-4 text-sm font-semibold text-red-700">
              <input type="checkbox" checked={values.isBanned} onChange={(event) => setField("isBanned", event.target.checked)} className="h-4 w-4 rounded border-red-300 text-red-600" />
              Banned akun admin
            </label>
          ) : (
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 px-6 py-4">
          ...
          <button type="submit" disabled={mutation.isPending} className="h-10 rounded-xl bg-teal-600 px-4 text-sm font-extrabold text-white disabled:opacity-60">{mutation.isPending ? "Menyimpan..." : "Simpan User"}</button>
        </div>
      </form>
    </CrudDialog>
  );
