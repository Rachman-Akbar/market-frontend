...
export const orderKeys = {
  all: ["order", "orderings"],
  customer: (userId, filters = {}) => ["order", "orderings", "customer", userId, filters],
  detail: (id) => ["order", "orderings", "detail", String(id || "")],
  shipping: (payload) => ["order", "shipping-options", payload],
  seller: (storeId, filters = {}) => ["order", "orderings", "seller", storeId, filters],
};
...
export function normalizeOrder(row = {}) {
  ...
  const subOrders = Array.isArray(row.sub_orders)
    ? row.sub_orders.map((subOrder) => ({
        ...
        items: Array.isArray(subOrder.items) ? subOrder.items.map(normalizeItem) : [],
      }))
    : [];
  ...
  return {
    ...
    subtotal: Number(row.subtotal ?? row.items_total ?? row.total_items_price ?? 0),
    totalAmount: Number(row.total_amount || 0),
    shippingCost: Number(row.shipping_cost || 0),
    discountAmount: Number(row.discount_amount || 0),
    shippingDiscountAmount: Number(row.shipping_discount_amount || 0),
    grandTotal: Number(row.grand_total ?? row.final_total ?? row.total_amount ?? 0),
    shippingAddress: row.shipping_address || "",
    courier: row.courier || "",
    service: row.service || "",
    snapToken: row.snap_token || row.midtrans_snap_token || null,
    items,
    subOrders,
...
function normalizePagination(payload) {
  ...
  return { data: rows.map(normalizeOrder), meta };
}

export async function getCustomerOrders(userId, filters = {}) {
  const response = await apiClient.get(`/api/v1/order/orderings/customers/${userId}`, { params: filters });
...
export async function getSellerOrders(storeId, filters = {}) {
  const response = await apiClient.get(`/api/v1/order/orderings/stores/${storeId}`, { params: filters });
...
export async function getShippingOptions(payload) {
  const response = await apiClient.post("/api/v1/order/orderings/shipping-options", {
    address_id: payload.addressId,
    cart_item_ids: payload.cartItemIds,
  });
  const source = unwrapApiData(response.data);
  return Array.isArray(source?.options) ? source.options : Array.isArray(source) ? source : [];
}
...
export function useOrderDetail(id) {
  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return order && ["pending", "unpaid"].includes(order.paymentStatus) ? 5000 : false;
    },
  });
}
...
export function useShippingOptions(payload, enabled = true) {
  return useQuery({
    ...
    enabled: Boolean(enabled && payload?.addressId && payload?.cartItemIds?.length),
    staleTime: 60000,
  });
}
