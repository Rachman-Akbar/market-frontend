    ...
    <CrudDialog open={open} onClose={onClose} title={entity ? "Edit Catalog Group" : "Tambah Catalog Group"} subtitle="Nama disimpan lowercase oleh backend untuk mencegah data ganda." size="max-w-xl">
      <form onSubmit={submit}>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 px-6 py-4">
          ...
          <button type="submit" disabled={mutation.isPending} className="h-10 bg-teal-600 px-4 text-sm font-extrabold text-white hover:bg-teal-700 disabled:opacity-60">{mutation.isPending ? "Menyimpan..." : "Simpan"}</button>
        </div>
      </form>
    </CrudDialog>
  );
