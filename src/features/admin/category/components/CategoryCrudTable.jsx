...
export const CategoryCrudTable = memo(function CategoryCrudTable({ rows, groupsById, onEdit, onToggleActive, pendingId, columns = CATEGORY_TABLE_COLUMNS, visibleSet, selectionEnabled = false, selectedIds = new Set(), allSelected = false, onToggleRow, onToggleAll }) {
  ...
  return (
    <div className="bg-white ring-1 ring-slate-200">
      <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-50 px-4 py-2.5"><div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-600">{[1, 2, 3].map((level) => <span key={level} className="bg-white px-2 py-1 ring-1 ring-inset ring-slate-200">Level {level}: {rows.filter((row) => getLevel(row) === level).length}</span>)}</div><div className="flex items-center gap-1"><button type="button" onClick={() => setExpandedIds(new Set(parentRows.map((row) => row.id)))} className="px-2 py-1 text-xs font-bold text-teal-700 hover:bg-teal-50">Buka Semua</button><button type="button" onClick={() => setExpandedIds(new Set())} className="px-2 py-1 text-xs font-bold text-slate-600 hover:bg-slate-100">Tutup Semua</button></div></div>
      <TableLayoutHint onReset={layout.resetLayout} />
      <div className="overflow-x-auto"><table className="table-fixed text-left text-sm" style={{ width: Math.max(tableWidth, 900), minWidth: "100%" }}><InteractiveColGroup columns={layout.orderedColumns} getColumnStyle={layout.getColumnStyle} leadingWidth={selectionEnabled ? 44 : 0} /><thead className="bg-slate-100 text-xs font-extrabold text-slate-600"><tr><TableSelectionHeader enabled={selectionEnabled} checked={allSelected} onToggle={onToggleAll} />{layout.orderedColumns.map((column) => <InteractiveTableHeader key={column.key} columnKey={column.key} headerProps={layout.getHeaderProps(column.key)} style={layout.getColumnStyle(column.key)} onResizeStart={layout.startResize} onResetWidth={layout.resetWidth} dragging={layout.dragKey === column.key} dropTarget={layout.dropKey === column.key} align={column.align}>{column.label}</InteractiveTableHeader>)}</tr></thead><tbody className="divide-y divide-slate-100">{visibleRows.map((row) => <tr key={row.id} onClick={() => onEdit(row)} className="cursor-pointer hover:bg-slate-50" title="Klik untuk edit"><TableSelectionCell enabled={selectionEnabled} checked={selectedIds.has(String(row.id))} onToggle={() => onToggleRow?.(row.id)} />{layout.orderedColumns.map((column) => renderCell(column, row))}</tr>)}</tbody></table></div>
    </div>
  );
});
