  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center gap-8 overflow-x-auto hide-scrollbar border-b border-[#bccbb4] pb-2">
        {tabs.map((tab, index) => (
          <button
            ...
            className={`whitespace-nowrap pb-2 text-base transition-colors ${index === activeTab ? "font-bold text-[#047857] border-b-2 border-[#047857]" : "text-[#3e4a39] hover:text-[#047857]"}`}
          >
            {tab}
          </button>
        ))}
      ...
      {!productsQuery.isLoading && !productsQuery.error && !products.length && (
        <div className="text-sm text-gray-500 py-8">Produk belum tersedia.</div>
      )}
      {!productsQuery.isLoading &&
        !productsQuery.error &&
        !!products.length && (
