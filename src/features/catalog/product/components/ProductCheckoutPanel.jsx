...
export function ProductCheckoutPanel({ product, variant }) {
  ...
  const handleAddCart = async () => {
    ...
    try {
      ...
      setMessage(getApiMessage(error, "Produk gagal ditambahkan ke keranjang."));
    }
  };
  ...
  const handleWishlist = async () => {
    ...
    try {
      ...
      setMessage(wished ? "Produk dihapus dari wishlist." : "Produk ditambahkan ke wishlist.");
    } catch (error) {
  ...
  return (
    <div className="sticky border border-gray-200 rounded-xl p-4 shadow-sm top-20">
      ...
      <div className="flex items-center gap-3 mb-4">
        <img src={image} alt={variantName} className="w-12 h-12 rounded-md object-cover" />
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex items-center border border-gray-200 rounded-lg h-9">
          <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="px-3 text-gray-300 font-bold hover:text-[#03ac0e]">-</button>
          <input type="text" value={qty} readOnly className="w-10 text-center border-none text-sm p-0 focus:ring-0" />
          <button onClick={() => setQty((q) => (stock ? Math.min(stock, q + 1) : q + 1))} className="px-3 text-[#03ac0e] font-bold">+</button>
        </div>
        <span className="text-sm">Stok: <span className="font-bold">{stock}</span></span>
      </div>
      ...
      <div className="space-y-2 mb-6">
        <button type="button" disabled={cartMutating || !stock} onClick={handleAddCart} className="w-full bg-[#03ac0e] text-white font-bold py-2.5 rounded-lg hover:bg-[#028a0b] transition-colors disabled:cursor-not-allowed disabled:opacity-60">
          {cartMutating ? "Menambahkan..." : "+ Keranjang"}
        </button>
      ...
      <div className="flex justify-around items-center text-xs font-bold text-gray-600">
        <button type="button" className="flex items-center gap-1.5 hover:text-[#03ac0e]">
        ...
        <button type="button" disabled={wishlistMutating} onClick={handleWishlist} className="flex items-center gap-1.5 hover:text-[#03ac0e] disabled:opacity-60">
          <svg className="w-4 h-4" fill={wished ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} /></svg>
          Wishlist
        </button>
        <div className="h-4 w-px bg-gray-200" />
        <button type="button" className="flex items-center gap-1.5 hover:text-[#03ac0e]">
