    ...
    variants.find((variant) => {
      return selectedKeys.every((key) => {
        return (variant.values || []).some(
          (item) =>
            String(item.attribute_id || item.attribute_name || "variant") ===
              String(key) && item.value === selectedValues[key],
        );
      });
    }) ||
...
export function VariantSelector({ variants = [], onVariantChange }) {
  ...
  useEffect(() => {
    ...
    groups.forEach((group) => {
      const matched = defaultVariant?.values?.find(
        (item) =>
          String(item.attribute_id || item.attribute_name || "variant") ===
          String(group.key),
      );
  ...
  useEffect(() => {
    ...
    const nextVariant = groups.length
      ? findVariant(variants, selectedValues)
            ...
            <button
              ...
              className={`min-w-[44px] h-11 px-3 rounded-lg text-sm font-semibold border-2 transition-colors ${
                variant.id === selectedVariantId
                  ? "border-[#10B981] bg-[#e5f9e7] text-[#10B981] font-bold"
            ...
            {group.values.map((item) => {
              ...
              return (
                <button
                  ...
                  className={
                    groupIndex === 0
                      ? `flex items-center gap-2 p-2 rounded-lg border-2 text-left ${active ? "border-[#10B981] bg-[#e5f9e7]" : "border-gray-200 hover:border-gray-400"}`
                      : `min-w-[44px] h-11 px-3 rounded-lg text-sm font-semibold border-2 transition-colors ${active ? "border-[#10B981] bg-[#e5f9e7] text-[#10B981] font-bold" : "border-gray-200 hover:border-[#10B981]"}`
                  }
                >
                  <span
                    className={
                      groupIndex === 0
                        ? `text-xs font-semibold ${active ? "text-[#10B981]" : "text-gray-500"}`
                        : ""
                    }
                  >
                    {item.label}
