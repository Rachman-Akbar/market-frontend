...
export default function WishlistPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return search ? items.filter((item) => `${item.productName} ${item.storeName}`.toLowerCase().includes(search)) : items;
  }, [items, keyword]);
      ...
      <div className={profileLayout.contentInner}>
        <div className={profileLayout.contentHeader}>
          <div>
            ...
            <p className={`mt-2 ${profileLayout.contentDesc}`}>Produk favorit yang tersimpan pada akun Anda.</p>
          </div>
          <label className={`${profileLayout.searchBox} w-full sm:max-w-xs`}>
            <Search size={17} className="mr-2" />
            <input value={keyword} onChange={(event) => setKeyword(event.target.value)} type="search" placeholder="Cari wishlist" className="w-full bg-transparent text-sm text-slate-700 outline-none placeholder:text-slate-400" />
          </label>
        </div>

        <hr className={profileLayout.divider} />

        {loading ? <p className="py-12 text-center text-sm text-slate-500">Memuat wishlist...</p> : null}
          ...
          <div key={item.productId} className="min-h-[104px] py-6">
            <div className="grid gap-4 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => removeItem(item.productId)} className={profileLayout.secondaryButton}>Hapus</button>
              </div>
            </div>
