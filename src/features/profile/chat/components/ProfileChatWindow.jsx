import { Lock, MessageCircle, Mic, MoreVertical, Plus, Search, Send, Smile, Video } from "lucide-react";
...
export function ProfileChatWindow({ thread }) {
  if (!thread) {
    return (
      <section className="relative flex h-full min-w-0 flex-1 flex-col bg-[#f7f8fa]" aria-label="Ruang chat kosong">
        <div className="flex h-full items-center justify-center p-8 text-center">
          <div className="max-w-md">
            ...
            <h2 className="mb-2 text-2xl font-light text-slate-950">Chat Marketplace</h2>
            <p className="mb-8 text-sm leading-6 text-slate-500">Pilih percakapan di sebelah kiri untuk mulai melihat pesan penjual, bantuan pembelian, atau update pesanan.</p>
      ...
      <header className={profileLayout.chatHeader}>
        <div className="flex min-w-0 items-center">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-slate-950">{thread.store}</h2>
            <p className="truncate text-xs text-slate-400">{thread.status === "online" ? "online" : "terakhir dilihat belum lama ini"}</p>
          </div>
        </div>
            ...
            return (
              <div key={message.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                <div className={cn("max-w-[78%] px-4 py-2 text-sm leading-6 shadow-sm", mine ? "rounded-l-2xl rounded-tr-2xl bg-[#d9fdd3] text-slate-900" : "rounded-r-2xl rounded-tl-2xl bg-white text-slate-800")}>
                  <p>{message.text}</p>
                  <time className={cn("mt-1 block text-right text-[10px] font-medium", mine ? "text-[#128c7e]" : "text-slate-400")}>{message.time}</time>
                </div>
              </div>
            );
          })}
