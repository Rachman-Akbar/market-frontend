...
export default function ShowcasePage() {
  ...
  const toggleActive = useCallback((row) => {
    saveMutation.mutateAsync({
      id: row.id,
      values: {
        ...(admin && row.store_id ? { store_id: Number(row.store_id) } : {}),
        name: row.name || "",
  ...
  const columns = useMemo(() => [
    ...
    { key: "is_active", label: "Status", width: 170, render: (row) => <span className="inline-flex"><InlineActiveSwitch checked={row.is_active !== false} pending={saveMutation.isPending} onChange={() => toggleActive(row)} compact /></span> },
    { key: "description", label: "Deskripsi", width: 300 },
  ], [saveMutation, toggleActive]);
