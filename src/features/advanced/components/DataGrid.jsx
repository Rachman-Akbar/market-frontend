export function DataGrid({ columns, rows, emptyText = "Data belum tersedia.", onRowClick, actions, selectionEnabled = false, selectedIds = new Set(), allSelected = false, onToggleRow, onToggleAll }) {
    ...
    <div className="w-full min-w-0 max-w-full max-h-[calc(100vh-285px)] overflow-auto border border-slate-200 bg-white">
      <table className="min-w-full border-collapse text-left text-sm">
        <thead className="sticky top-0 z-10 bg-slate-100 text-xs uppercase tracking-wide text-slate-600">
          <tr>
            {selectionEnabled ? (
              <th className="w-10 border-b border-slate-200 px-3 py-2.5 text-center">
                <input type="checkbox" checked={allSelected} onChange={onToggleAll} aria-label="Pilih semua data" />
              </th>
            ) : null}
            {columns.map((column) => <th key={column.key} className="whitespace-nowrap border-b border-slate-200 px-3 py-2.5 font-extrabold">{column.label}</th>)}
        ...
        <tbody className="divide-y divide-slate-100">
          {rows.map((row) => (
            <tr key={row.id} onClick={() => onRowClick?.(row)} className={onRowClick ? "cursor-pointer hover:bg-emerald-50/40" : "hover:bg-slate-50"}>
              ...
              {columns.map((column) => <td key={column.key} className="max-w-[300px] px-3 py-2.5 align-top text-slate-700">{column.render ? column.render(row) : String(row[column.key] ?? "-")}</td>)}
