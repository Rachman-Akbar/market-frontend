      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <h3 className="truncate text-sm font-extrabold text-slate-900">{item.title}</h3>
      ...
      <div className={cn("max-h-[min(60vh,26rem)] flex-1 overflow-y-auto", contentClassName)}>
        {rows.length ? rows.map((item) => <NotificationItem key={item.id} item={item} onRemove={center.remove} />) : (
          <div className="flex min-h-40 flex-col items-center justify-center px-6 py-10 text-center">
            ...
            <p className="mt-2 text-sm font-bold text-slate-600">Belum ada {center.activeTab === "queue" ? "proses dalam antrean" : "informasi"}.</p>
          </div>
        )}
      </div>
    </section>
