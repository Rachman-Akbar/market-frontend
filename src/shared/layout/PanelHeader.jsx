        ...
        <div className="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          {searchPlaceholder ? (
            <div className="hidden min-w-0 flex-1 justify-center md:flex">
              <div className={cn(
                "flex w-full max-w-xl items-center rounded-lg bg-slate-50 px-3 py-2 focus-within:bg-white",
                focusClassName,
              )}>
          ...
          <div className="flex items-center gap-2">
            {actionHref ? (
              <Link
                to={actionHref}
                className={cn("hidden rounded-lg bg-white px-3 py-2 text-xs font-extrabold text-slate-700 transition sm:inline-flex", actionClassName)}
              >
                {actionLabel}
              </Link>
            ) : null}
            <button
              type="button"
              className={cn("flex h-10 w-10 items-center justify-center rounded-lg bg-white text-slate-600 transition", notificationClassName)}
