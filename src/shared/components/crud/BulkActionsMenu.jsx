...
export const BulkActionsMenu = memo(function BulkActionsMenu({ selectedCount = 0, actions = [], disabled = false }) {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <div className="absolute right-0 top-full z-[110] mt-1 min-w-56 bg-white py-1 ring-1 ring-slate-200">
          {availableActions.map((action) => (
            <button
              ...
              className={`flex w-full items-center gap-3 px-3 py-2.5 text-left text-sm font-bold disabled:opacity-40 ${action.danger ? "text-red-600 hover:bg-red-50" : "text-slate-700 hover:bg-slate-50"}`}
            >
