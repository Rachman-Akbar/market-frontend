      ...
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        ...
        {onToggleSelection ? (
          <button
            ...
            className={`inline-flex h-10 items-center justify-center gap-2 px-3 text-sm font-bold transition-colors ${selectionEnabled ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
          >
            <span className="material-symbols-outlined text-[19px]">check_box</span>
            <span className="hidden md:inline">Pilih Data</span>
          </button>
        ) : null}
        ...
        {!hideCreate ? (
          <button
            type="button"
            onClick={onCreate}
            className="inline-flex h-10 items-center justify-center gap-2 bg-emerald-600 px-4 text-sm font-extrabold text-white hover:bg-emerald-700"
          >
